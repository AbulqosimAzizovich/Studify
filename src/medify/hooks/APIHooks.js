...
import { isRequestSuccessful, sanitizeData } from "../helpers/ApiHelper";
...
export const useGetDataApi = (url, initialData = undefined, params = {}, initialCall = true, callbackFun) => {
  ...
  useEffect(() => {
    const fetchData = () => {
      ...
      if (
        skip === 0 &&
        ((Array.isArray(apiData) && apiData.length === 0) || !Array.isArray(apiData)) &&
        !isResetRequired
      ) {
        setLoading(true);
      }
...
export const postDataApi = (url, infoViewContext, payload, isHideLoader = false, headers) => {
  ...
  return new Promise((resolve, reject) => {
    console.log(fetchStart, fetchSuccess, headers);
...
export const downloadDataApi = (url, infoViewContext, params = {}, isHideLoader = false) => {
  const { fetchStart, fetchSuccess } = infoViewContext;
  return new Promise((resolve, reject) => {
    console.log(fetchStart, fetchSuccess, headers);
    if (!isHideLoader) fetchStart();
    ...
    //     } else {
    //       const data = JSON.parse(String.fromCharCode.apply(null, new Uint8Array(res.data)));
    //       return reject(data);
    //     }
    //   })
    //   .catch((error) => {
